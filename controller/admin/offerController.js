...
const offerPageLoad = asyncHandler(async (req, res) => {
  ...
  res.render("admin/errorPage", { message: "An error occurred while loading " });
  }
});
...
const addOffer = asyncHandler(async (req, res) => {
  try {
    const { type, name, description, offerPercentage, validFrom, validity } =
      req.body;
    ...
    if (isNaN(percentage) || percentage <= 0 || percentage > 100) {
      return res
        .status(400)
        .json({ error: "Offer percentage must be between 1 and 100" });
    }
    let existingOffer;
    if (type === "Products") {
    ...
    return res.render("admin/errorPage", { message:`An active offer already exists for this ${type.toLowerCase()}`});
    }
    ...
    res.render("admin/errorPage", { message: "Error while creating an offer " });
  }
});
...
const deleteOffer = async (req, res) => {
  try {
    ...
    if (!offer) {
      return res.render("admin/errorPage", { message: "offer not found " });
    }
    res.json({ success: true, message: "Offer deleted successfully" });
  } catch (error) {
    res.render("admin/errorPage", { message: "Error while deleting the offer" });
  }
};
